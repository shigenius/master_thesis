...
flags.DEFINE_string('output_dir', './data/', 'Path to dir of output tf-record')
...
def main(_):
  ...
  for year in years:
    ...
    # examples_list_ = dataset_util.read_examples_list(examples_path)
    #
    # # 1000itemごとにファイルを分割する
    # split_examples_list = list(split_list(examples_list_, 1000))
    # num_files = len(split_examples_list)
    # for i in range(num_files):
    #     examples_list = split_examples_list[i] # 0~1000 record
    #
    #     output_file_name = "%s-%s.tfrecord-%.5d-of-%.5d" % (year, FLAGS.set, i, num_files)
    #     output_file_path = os.path.join(FLAGS.output_dir, output_file_name)

    output_file_name = "%s-%s.tfrecord" % (year, FLAGS.set)
    output_file_path = os.path.join(FLAGS.output_dir, output_file_name)
